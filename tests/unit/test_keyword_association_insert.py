...
class TestRouteCallSite:
    def test_route_calls_the_existing_method_inside_its_own_try(self):
        ...
        # pr8-clean keeps main's hash_id flow: the route bulk-inserts through
        # the keywords_hashs repository (the insert that exists on this lineage).
        assert 'bulk_insert_keywords_hashs(hash_id, keyword_counts)' in source
